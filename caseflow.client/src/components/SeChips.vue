<template>
  ...
  <span v-else-if="names.length === 1" class="text-slate-700 text-[13px] break-words [overflow-wrap:anywhere]">{{ names[0] }}</span>

  <span v-else class="inline-flex items-center gap-1 group/se relative min-w-0 max-w-full">
    <span class="text-slate-700 text-[13px] break-words [overflow-wrap:anywhere] min-w-0">{{ names[0] }}</span>
    ...
    <span class="absolute left-0 top-full mt-1.5 hidden group-hover/se:block z-20 min-w-[160px] max-w-[260px] bg-slate-900 text-black text-[11px] rounded-lg shadow-lg p-2.5">
      <span class="block text-[10px] text-slate-400 mb-1">共 {{ names.length }} 位專案成員</span>
      <span v-for="(name, index) in names" :key="`${name}-${index}`" class="py-0.5 flex items-center gap-1.5">
        <span class="w-1 h-1 rounded-full bg-slate-500"></span>
        <span class="break-words [overflow-wrap:anywhere]">{{ name }}</span>
      </span>
