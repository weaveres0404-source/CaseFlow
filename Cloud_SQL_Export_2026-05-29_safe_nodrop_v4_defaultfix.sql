...
CREATE TABLE public.users (
    ...
    role character varying(20) DEFAULT 'SE' CONSTRAINT "Users_role_not_null" NOT NULL,
    ...
    CONSTRAINT check_role_types CHECK (((role)::text = ANY (ARRAY[('SysAdmin'::character varying)::text, ('PM'::character varying)::text, ('SE'::character varying)::text])))
);
...
CREATE TABLE public.attachments (
    ...
    CONSTRAINT attachments_entity_type_check CHECK (((entity_type)::text = ANY (ARRAY[('case'::character varying)::text, ('case_log'::character varying)::text, ('case_reply'::character varying)::text, ('case_estimation'::character varying)::text])))
);
...
CREATE TABLE public.cases (
    ...
    case_type character varying(20) DEFAULT 'REPAIR' NOT NULL,
    ...
    CONSTRAINT cases_case_type_check CHECK (((case_type)::text = ANY (ARRAY[('REPAIR'::character varying)::text, ('EVALUATION'::character varying)::text, ('MAINTENANCE'::character varying)::text, ('UHD'::character varying)::text, ('INQUIRY'::character varying)::text]))),
    CONSTRAINT cases_priority_check CHECK (((priority)::text = ANY (ARRAY[('HIGH'::character varying)::text, ('MEDIUM'::character varying)::text, ('LOW'::character varying)::text]))),
    CONSTRAINT cases_relation_type_check CHECK (((relation_type)::text = ANY (ARRAY[('REOPEN'::character varying)::text, ('RELATED'::character varying)::text]))),
...
CREATE TABLE public.project_members (
    ...
    member_role character varying DEFAULT 'SE' CONSTRAINT member_member_role_not_null NOT NULL,
...
CREATE TABLE public.notifications (
    ...
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT notifications_notification_type_check CHECK (((notification_type)::text = ANY (ARRAY[('CASE_CREATED'::character varying)::text, ('CASE_ASSIGNED'::character varying)::text, ('WORK_COMPLETED'::character varying)::text, ('CASE_RETURNED'::character varying)::text, ('ESTIMATION_DONE'::character varying)::text, ('CASE_CANCELLED'::character varying)::text, ('CASE_CLOSED'::character varying)::text, ('CASE_REOPENED'::character varying)::text])))
);
...
CREATE TABLE public.project_codes (
    ...
    sort_order integer DEFAULT 0 NOT NULL,
    is_active boolean DEFAULT true NOT NULL,
    created_at timestamp without time zone DEFAULT (now() AT TIME ZONE 'Asia/Taipei'::text) NOT NULL,
    updated_at timestamp without time zone DEFAULT (now() AT TIME ZONE 'Asia/Taipei'::text) NOT NULL
);
